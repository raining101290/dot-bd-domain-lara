<div class="top-header">
    <div class="header-bar d-flex justify-content-between">
        <div class="d-flex align-items-center">
            <a href="#" class="logo-icon me-3">
                <img src="/backend/assets/images/logo.png" height="30" class="small" alt="">
                <span class="big">
                    <img src="/backend/assets/images/logo.png" height="24" class="logo-light-mode"
                        alt="">
                    <img src="/backend/assets/images/logo.png" height="24" class="logo-dark-mode"
                        alt="">
                </span>
            </a>
            <a id="close-sidebar" class="btn btn-icon btn-soft-light" href="javascript:void(0)">
                <i class="ti ti-menu-2"></i>
            </a>
        </div>

        <ul class="list-unstyled mb-0">
            <li class="list-inline-item mb-0 ms-1">
                <div class="dropdown dropdown-primary">
                    <button type="button" class="btn btn-soft-light dropdown-toggle p-0"
                        data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <img src="/backend/assets/images/client/imran-gray.jpeg"
                            class="avatar avatar-ex-small rounded" alt="">
                    </button>
                    <div class="dropdown-menu dd-menu dropdown-menu-end shadow border-0 mt-3 py-3"
                        data-popper-placement="bottom-end">
                        <a class="dropdown-item d-flex align-items-center text-dark pb-3" href="/customer/profile">
                            <img src="/backend/assets/images/client/imran-gray.jpeg" class="avatar avatar-md-sm rounded-circle border shadow" alt="" />
                            <div class="flex-1 ms-2">
                                <span class="d-block">Imran</span>
                                <small class="text-muted">Software Engineer</small>
                            </div>
                        </a>
                        <a class="dropdown-item text-dark" href="/customer/dashboard">
                            <span class="mb-0 d-inline-block me-1">
                                <i class="ti ti-home"></i>
                            </span> 
                            Dashboard
                        </a>
                        <a class="dropdown-item text-dark" href="/customer/profile">
                            <span class="mb-0 d-inline-block me-1">
                                <i class="ti ti-user"></i>
                            </span>
                            Profile
                        </a>
                        <a class="dropdown-item text-dark" href="/customer/profile#security">
                            <span class="mb-0 d-inline-block me-1">
                                <i class="ti ti-settings"></i>
                            </span>
                            Chnage Password
                        </a>
                        <div class="dropdown-divider border-top"></div>
                        <a class="dropdown-item text-dark" href="#" id="logoutBtn">
                            <span class="mb-0 d-inline-block me-1">
                                <i class="ti ti-logout"></i>
                            </span> 
                            Logout
                        </a>
                    </div>
                </div>
            </li>
        </ul>
    </div>
</div>